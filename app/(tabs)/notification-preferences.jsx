...
export default function NotificationPreferencesScreen() {
  ...
  return (
    <AppLayout showNavbar={false} className={isDark ? 'bg-black' : 'bg-white'}>
      <View className={`flex-1 ${isDark ? 'bg-black' : 'bg-white'}`}>
        <View
          className={`border-b px-5 pb-4 pt-4 ${isDark ? 'border-white/10 bg-black' : 'border-black/[0.08] bg-white'
            }`}
        >
          <View className="flex-row items-center">
            ...
            <Text className="text-lg font-bold text-black dark:text-white">Notification preferences</Text>
          </View>
        </View>
        ...
        >
          ...
          <Pressable
            ...
            className={`mt-6 flex-row items-center justify-between rounded-2xl border px-4 py-4 ${isDark ? 'border-white/10 bg-white/[0.06]' : 'border-black/[0.08] bg-black/[0.02]'
              }`}
          >
          ...
          <Pressable
            ...
            className={`mt-3 flex-row items-center justify-between rounded-2xl border px-4 py-4 ${isDark ? 'border-white/10 bg-white/[0.06]' : 'border-black/[0.08] bg-black/[0.02]'
              }`}
          >
            <View className="flex-row items-center gap-3 flex-1 pr-3">
              <View className="h-10 w-10 items-center justify-center rounded-xl bg-alpha/15">
          ...
          <View
            className={`overflow-hidden rounded-3xl border ${isDark ? 'border-white/10 bg-[#1A1816]' : 'border-black/[0.06] bg-white'
              }`}
          >
