...
| `app/(tabs)/profile/partials/` | Profile screen layout: header, tabs, grids, follow list, feed modals |
| `app/(tabs)/profile/components/` | Reusable profile modals (`EditProfileModal`, experience/education forms) — used by profile + More |
...
`components/` keeps cross-feature reusable UI only: `ui/`, `layout/`, `helpers/`, `legal/`, call widgets, gates. Profile-specific modals are under `app/(tabs)/profile/components/`.
...
Profile tab is now `app/(tabs)/profile/index.jsx` (replaces `profile.jsx`).
